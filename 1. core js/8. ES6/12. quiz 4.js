...
//연습 2 _ 나와 거의 모든부분 긱포긱
const mappedAllCity = traders
.map( user => {
  return user.trader.city;
})
console.log(removeDuplicates(mappedAllCity));
...
console.log(removeDuplicates(allUserName));

//연습 5 
// let total = 0;
// traders.filter (user => user.trader.city === '서울')
// .map (user => user.value)
// .forEach ( v =>  total += +v);
