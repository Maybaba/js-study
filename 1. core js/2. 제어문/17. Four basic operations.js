...
//입력받은 값이 실제 답과 같을때까지 반복하기
while (trueAnswer === answer) {  
  //두 개의 랜덤 숫자를 고르기 [1~20]
  var rn1 = Math.floor(Math.random(20)) + 1;
  var rn2 = Math.floor(Math.random(20)) + 1;
  //두개의 랜덤 숫자를 더한 답 변수선언하기
  var trueAnswer = rn1 + rn2; 
  // 답이 무엇인지 입력받기
  var answer = +prpmpt (`Q. ${rn1} + ${rn2} = ??? ${trueAnswer}`);
...
count ++
//두 개의 랜덤 숫자를 더해서 출력하기
console.log(`Q1. ${rn1} + ${rn2} = ??? ${trueAnswer}`);
